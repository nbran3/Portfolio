...
ggplot(data, aes(x = UNEMPLOYMENT, y = CPI.US)) +
  geom_point() +
  geom_abline(intercept = coef(model2)[1], slope = coef(model2)[2], color = "red") +
  xlab("Unemployment") +
  ylab("CPI") +
  theme_bw() +
...
qqnorm(residuals1)
```
## CPI, Unemployment, FFR Multivariate : This is a multivariate linear regression model, with CPI being the depedent variable and Unemployment and the FFR being the indepedents. Below we can an R-Sqaured of .1868, which is actually higher than the R-Sqaured values of the previous model. Which means that 18.68% of the variation in CPI can be explained by variation in unemployment and the federal funds rate at the same time. Albeit it weak and a p-value of something in the scientfic notation, and a rather high f-stat of 86.97, we can conclude that the relationship between the three variables is statsically signficant.
...
```
```{r}
render("EconIndicatorsMark.Rmd", output_format = "pdf_document")
```
